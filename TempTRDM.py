...
def compare_path(path1,path2):
    if len(path1)<len(path2):
        ...
        for i in range(len(path1)):
            if path1[i]!=path2[i]:
                first_non_match.append(path1[i])
                first_non_match.append(path2[i])
                break
        ...
        subgraphs = []
        shortest_paths=[]
        shortest_paths.append(path1)
        shortest_paths.append(path2)
        for i in range(len(first_non_match)):
            ...
            nodes_to_remove = []
            # print('aaaaa',shortest_paths[i][shortest_paths[i].index(first_non_match[i])+1:])
            ...
            nodes_to_remove.append(first_non_match[i])
...
def choose_shortest_path(shortest_paths):
    ...
    ret_path = shortest_paths[0]
    # print('dddddd',len(shortest_paths))
    for i in range(1,len(shortest_paths)):
        ret_path = compare_path(ret_path,shortest_paths[i])

    # print('final_path:',ret_path)
    return ret_path
